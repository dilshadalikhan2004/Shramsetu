...
export default function ChatPage() {
    ...
    const {
        workerConversations: conversations,
        workerMessages: allMessages,
        sendWorkerMessage,
        markWorkerConvoRead,
    } = useChatStore();
    ...
    }, [active, mounted, markWorkerConvoRead]);
    ...
    return (
        <div className="flex h-full bg-white rounded-2xl border overflow-hidden" style={{ borderColor: "#e5e7eb", height: "calc(100vh - 120px)" }}>
                    ...
                    {filtered.map((c) => (
                        <button key={c.id} onClick={() => openChat(c.id)}
                            ...
                            <div className="relative shrink-0">
                                <div className="w-11 h-11 rounded-full flex items-center justify-center font-outfit font-bold text-white text-sm"
                                    style={{ background: c.color }}>{c.init}</div>
                                {c.online && <span className="absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white" style={{ background: "#0e9f6e" }} />}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-baseline">
                                    <p className="font-dmsans font-bold truncate" style={{ fontSize: 14, color: active === c.id ? "#e85d26" : "#111827" }}>{c.name}</p>
                                    <span style={{ fontSize: 11, color: "#9ca3af", flexShrink: 0, marginLeft: 4 }}>{c.time}</span>
                                </div>
                            ...
                            {c.unread > 0 && (
                                <span className="w-5 h-5 rounded-full flex items-center justify-center text-white font-bold text-xs shrink-0"
                                    style={{ background: "#e85d26" }}>{c.unread}</span>
                            )}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 px-5 py-3.5 border-b bg-white shrink-0" style={{ borderColor: "#e5e7eb" }}>
                        <div className="relative">
                            <div className="w-10 h-10 rounded-full flex items-center justify-center font-outfit font-bold text-white text-sm"
                                style={{ background: activeConvo.color }}>{activeConvo.init}</div>
                        ...
                        <div className="flex items-center gap-2">
                            <button className="w-9 h-9 rounded-xl flex items-center justify-center transition-all hover:bg-gray-100" style={{ background: "#f3f4f6" }}>
                                <Phone className="w-4 h-4" style={{ color: "#374151" }} />
                            </button>
                            <button className="w-9 h-9 rounded-xl flex items-center justify-center transition-all hover:bg-gray-100" style={{ background: "#f3f4f6" }}>
                                <Video className="w-4 h-4" style={{ color: "#374151" }} />
                            </button>
                            <button className="w-9 h-9 rounded-xl flex items-center justify-center transition-all hover:bg-gray-100" style={{ background: "#f3f4f6" }}>
                                <MoreVertical className="w-4 h-4" style={{ color: "#374151" }} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3" style={{ background: "#f8fafc" }}>
                        <div className="text-center mb-2">
                            <span className="px-3 py-1 rounded-full font-dmsans text-xs" style={{ background: "#e5e7eb", color: "#6b7280" }}>{t('chat.today')}</span>
                        </div>
                        {messages.map((m) => (
                    ...
                    <div className="flex items-center gap-3 px-5 py-4 border-t bg-white shrink-0" style={{ borderColor: "#e5e7eb" }}>
                        ...
                        <button onClick={send} className="w-11 h-11 rounded-xl flex items-center justify-center transition-all active:scale-95"
